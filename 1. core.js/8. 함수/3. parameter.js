...
// 인수(argument) : 함수를 호출할 때 함수에게 전달하는 3, 7같은 값들.
...
console.log(`reult1: ${calcRangeTotal(1, 100)}`);
...
// 매개변수가 없는 함수
function selecRandomPet() {
    var pets = ['멍멍이', '야옹이', '짹짹이', '꽥꽥이', '어흥이'];

    // 0 ~ length-1 까지의 랜덤 정수
    var rn = Math.floor(Math.random() * pets.length);
    return pets[rn];
}

console.log(`선택된 동물: ${selecRandomPet()}`);
